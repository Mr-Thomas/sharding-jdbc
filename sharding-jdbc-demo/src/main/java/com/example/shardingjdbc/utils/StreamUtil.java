...
public class StreamUtil {
    ...
    private static <T, C extends Collection<T>> C stringToCollection(String source, String divider, Function<? super String, ? extends T> keyMapper, Supplier<C> supplier) {
        if (StringUtils.isNotEmpty(source) && StringUtils.isNotEmpty(divider)) {
            ...
            return Arrays.stream(source.split(divider)).map(String::trim).filter(StringUtils::isNotEmpty).map(keyMapper).collect(Collectors.toCollection(supplier));
        }
        return supplier.get();
    }
     ...
     */
    public static <T, R , C extends Collection<T>> String collectionToString(C source, String divider, Function<? super T, ? extends R> keyMapper) {
        if (CollectionUtils.isNotEmpty(source)) {
            if (Objects.isNull(keyMapper)) {
                keyMapper = k -> (R) k;
            }
            String result = source.stream().map(keyMapper).filter(Objects::nonNull).map(Object::toString).filter(StringUtils::isNotEmpty).reduce("", (a, b) -> a + divider + b);
            return trim(result, divider);
        }
        return "";
    }

    public static <T, R , C extends Collection<T>> String collectionToString(C source, String divider) {
        return collectionToString(source, divider, null);
    }

    public static <T, C extends Collection<T>> T findFirst(C data) {
        if (CollectionUtils.isNotEmpty(data)) {
            Optional<T> optional = data.stream().findFirst();
